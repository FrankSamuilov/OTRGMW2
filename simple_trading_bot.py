...
class SimpleTradingBot:
    ...
    async def _run_integrated_analysis(self, account_balance: float):
        """
        运行整合分析 - 结合博弈论和技术分析
        """
        ...
        if trading_opportunities:
            ...
            # 执行交易
            print_colored(f"\n💫 准备执行交易...", Colors.CYAN)
            await self._execute_integrated_trade(best_opportunity, account_balance)
    ...
    async def _perform_technical_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if trend_dir == 'UP':
                ...
                signal_details.append("横盘整理 (0.0)")
            ...
            if 'rsi' in technical_signals['momentum']:
                ...
                if rsi < 20:  # 极度超卖
                    ...
                    print_colored(f"    • RSI极度超卖 ({rsi:.1f}, 得分: +{rsi_score:.1f})", Colors.GREEN + Colors.BOLD)
                elif rsi < 30:  # 超卖
                    rsi_score = 1.5
                    signal_details.append(f"RSI超卖 (+{rsi_score:.1f})")
                    print_colored(f"    • RSI超卖 ({rsi:.1f}, 得分: +{rsi_score:.1f})", Colors.GREEN)
                elif rsi < 40:  # 偏低
                    rsi_score = 0.5
                    signal_details.append(f"RSI偏低 (+{rsi_score:.1f})")
                    print_colored(f"    • RSI偏低 ({rsi:.1f}, 得分: +{rsi_score:.1f})", Colors.YELLOW)
                elif rsi > 80:  # 极度超买
                    rsi_score = -2.0
                    signal_details.append(f"RSI极度超买 ({rsi_score:.1f})")
                    print_colored(f"    • RSI极度超买 ({rsi:.1f}, 得分: {rsi_score:.1f})", Colors.RED + Colors.BOLD)
                elif rsi > 70:  # 超买
                    rsi_score = -1.5
                    signal_details.append(f"RSI超买 ({rsi_score:.1f})")
                    print_colored(f"    • RSI超买 ({rsi:.1f}, 得分: {rsi_score:.1f})", Colors.RED)
                elif rsi > 60:  # 偏高
                    rsi_score = -0.5
                    signal_details.append(f"RSI偏高 ({rsi_score:.1f})")
            ...
            if sr.get('supports') or sr.get('resistances'):
                ...
                # 检查是否接近支撑位
                if sr.get('supports'):
                    nearest_support = sr['supports'][-1] if sr['supports'] else 0
                    if nearest_support > 0:
                        support_distance = (price - nearest_support) / price
                        if support_distance < 0.01:  # 接近支撑位1%以内
                            support_score = 0.5
                            signal_count += support_score
                            signal_details.append(f"接近支撑位 (+{support_score:.1f})")
                            print_colored(f"    • 接近支撑位 ${nearest_support:.2f} (得分: +{support_score:.1f})",
                                          Colors.GREEN)

                # 检查是否接近阻力位
                if sr.get('resistances'):
                    nearest_resistance = sr['resistances'][0] if sr['resistances'] else 0
                    if nearest_resistance > 0:
                        resistance_distance = (nearest_resistance - price) / price
                        if resistance_distance < 0.01:  # 接近阻力位1%以内
                            resistance_score = -0.5
                            signal_count += resistance_score
                            signal_details.append(f"接近阻力位 ({resistance_score:.1f})")
                            print_colored(f"    • 接近阻力位 ${nearest_resistance:.2f} (得分: {resistance_score:.1f})",
                                          Colors.RED)
    ...
    def _integrate_analyses(self, game_theory: Dict, technical: Dict, symbol: str) -> Dict[str, Any]:
        ...
        whale_intent = game_theory.get('whale_intent', 'NEUTRAL')
        whale_confidence = game_theory.get('confidence', 0)
        tech_strength = technical.get('signal_strength', 0)
    ...
    def _integrate_analyses_trend_first(self, game_theory: Dict, technical: Dict, symbol: str) -> Dict[str, Any]:
        ...
        ema20 = technical.get('ema20', price)
        ema50 = technical.get('ema50', price)
        bb_position = technical.get('bb_position', 50)  # 布林带位置百分比
        ...
        # 2. 布林带位置
        if bb_position > 100:  # 价格在布林带上轨之上
            trend_score += 30
            trend_signals.append("突破布林带上轨")
        elif bb_position > 80:
            trend_score += 15
            trend_signals.append("接近布林带上轨")
        elif bb_position < 0:  # 价格在布林带下轨之下
            trend_score -= 30
            trend_signals.append("跌破布林带下轨")
        elif bb_position < 20:
            trend_score -= 15
            trend_signals.append("接近布林带下轨")

        # 3. 威廉指标（修正解读）
        if williams_r > -20:  # 接近0表示强势
            trend_score += 20
            trend_signals.append("威廉指标强势")
        elif williams_r < -80:  # 接近-100表示弱势
            trend_score -= 20
            trend_signals.append("威廉指标弱势")

        # 4. 动量指标
        if 'momentum' in technical and technical['momentum'] > 0:
            trend_score += 10
            trend_signals.append("正向动量")
        elif 'momentum' in technical and technical['momentum'] < 0:
            trend_score -= 10
            trend_signals.append("负向动量")

        # ========== 第二步：根据趋势调整RSI解读 ==========
        rsi_signal = 0
        if trend_score > 20:  # 上涨趋势
            if 50 <= rsi <= 70:
                rsi_signal = 20  # 健康上涨
                integrated['signals'].append(f"RSI健康区间({rsi:.1f})")
            elif rsi > 70:
                rsi_signal = -5  # 轻微警告，不是强烈看空
                integrated['signals'].append(f"RSI偏高({rsi:.1f})，但趋势仍强")
            elif rsi < 50:
                rsi_signal = 30  # 回调买入机会
                integrated['signals'].append(f"RSI回调({rsi:.1f})，买入机会")
        elif trend_score < -20:  # 下跌趋势
            if 30 <= rsi <= 50:
                rsi_signal = -20  # 健康下跌
                integrated['signals'].append(f"RSI确认下跌({rsi:.1f})")
            elif rsi < 30:
                rsi_signal = 5  # 可能反弹
                integrated['signals'].append(f"RSI超卖({rsi:.1f})，可能反弹")
            elif rsi > 50:
                rsi_signal = -30  # 反弹做空机会
                integrated['signals'].append(f"RSI反弹({rsi:.1f})，做空机会")
        else:  # 震荡市
            if rsi > 70:
                rsi_signal = -20
                integrated['signals'].append(f"RSI超买({rsi:.1f})")
            elif rsi < 30:
                rsi_signal = 20
                integrated['signals'].append(f"RSI超卖({rsi:.1f})")
        ...
        game_signal = 0
        if whale_intent == 'ACCUMULATION':
            game_signal = 20 * whale_confidence
            integrated['signals'].append("检测到吸筹")
        elif whale_intent == 'DISTRIBUTION':
            # 只有在下跌趋势中，派发才是强烈信号
            if trend_score < -20:
                game_signal = -30 * whale_confidence
                integrated['signals'].append("趋势向下+庄家派发")
            else:
                game_signal = -10 * whale_confidence  # 上涨中的派发可能是正常获利了结
                integrated['risks'].append("可能有获利了结")

        # ========== 第四步：综合评分 ==========
        # 趋势权重60%，RSI权重25%，博弈论权重15%
        final_score = trend_score * 0.6 + rsi_signal * 0.25 + game_signal * 0.15

        # ========== 第五步：生成交易决策 ==========
        # 明确的趋势跟随逻辑
        if final_score > 25:
            integrated['action'] = 'BUY'
            integrated['confidence'] = min(final_score / 100, 0.85)
            integrated['strategy_type'] = 'TREND_FOLLOWING'
            integrated['reason'] = f"上涨趋势明确 ({', '.join(trend_signals[:3])})"
        elif final_score < -25:
            integrated['action'] = 'SELL'
            integrated['confidence'] = min(abs(final_score) / 100, 0.85)
            integrated['strategy_type'] = 'TREND_FOLLOWING'
            integrated['reason'] = f"下跌趋势明确 ({', '.join(trend_signals[:3])})"
        else:
            integrated['action'] = 'HOLD'
            integrated['reason'] = "趋势不明确或信号不足"
            integrated['strategy_type'] = 'WAIT_AND_SEE'

        # ========== 第六步：安全检查 ==========
        # 防止在极端情况下逆势交易
        if integrated['action'] == 'SELL' and bb_position > 100:
            # 价格在布林带上轨之上还要做空？重新评估
            integrated['action'] = 'HOLD'
            integrated['reason'] = "价格突破上轨，暂不做空"
            integrated['risks'].append("避免追高做空")
        elif integrated['action'] == 'BUY' and bb_position < 0:
            # 价格在布林带下轨之下还要做多？重新评估
            integrated['action'] = 'HOLD'
            integrated['reason'] = "价格跌破下轨，暂不做多"
            integrated['risks'].append("避免抄底")

        # 设置最终参数
        integrated['final_score'] = abs(final_score) / 10
        integrated['game_theory_analysis'] = game_theory
        integrated['technical_analysis'] = technical

        # 详细输出
        print_colored(f"\n🔗 趋势优先整合分析:", Colors.CYAN + Colors.BOLD)
        print_colored(f"    • 趋势得分: {trend_score:.1f} (权重: 60%)", Colors.INFO)
        print_colored(f"    • RSI信号: {rsi_signal:.1f} (权重: 25%)", Colors.INFO)
        print_colored(f"    • 博弈信号: {game_signal:.1f} (权重: 15%)", Colors.INFO)
        print_colored(f"    • 最终得分: {final_score:.1f}", Colors.CYAN)
        print_colored(f"    • 交易决策: {integrated['action']} (置信度: {integrated['confidence']:.1%})",
                      Colors.GREEN if integrated['action'] == 'BUY' else Colors.RED if integrated[
                                                                                           'action'] == 'SELL' else Colors.YELLOW)
    ...
    def _analyze_momentum(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'STOCHk' in df.columns:
            momentum['stoch_k'] = df['STOCHk'].iloc[-1]
            momentum['stoch_d'] = df['STOCHd'].iloc[-1] if 'STOCHd' in df.columns else 0
    ...
    def _analyze_volume(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'volume' in df.columns:
            ...
            volume_analysis['average'] = avg_vol
            volume_analysis['ratio'] = current_vol / avg_vol if avg_vol > 0 else 0

            # 成交量趋势
            vol_trend = df['volume'].rolling(5).mean().iloc[-1] > df['volume'].rolling(20).mean().iloc[-1]
            volume_analysis['trend'] = 'INCREASING' if vol_trend else 'DECREASING'
    ...
    async def _execute_integrated_trade(self, opportunity: Dict, account_balance: float):
        """执行整合分析后的交易 - 智能最小订单版本"""
        try:
    ...
    async def _run_game_theory_analysis(self, account_balance: float):
        """
        ...
        for symbol in self.config["TRADE_PAIRS"]:
            ...
            try:
                ...
                market_data = await self.data_collector.collect_full_market_data(symbol)
